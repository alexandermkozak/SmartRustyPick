 ...
 *   at module scope - as `useServerStats` does, to share one poll across views -
 *   the caller drives `start` and `stop` instead, so unmounting the view that
 *   happened to be first does not stop everyone else's data.
 */
