...
fn rows_for(db: &Database, entries: Vec<SelectEntry>) -> Vec<(SelectEntry, &Record)> {
    ...
    entries.into_iter().map(|e| {
        let record = table.records.get(&e.key).unwrap();
        (e, record)
    }).collect()
}
...
fn test_render_list_headers_and_widths() {
    ...
    let lines = report::render_list(&db, "USERS", &["NAME".to_string()], None, &rows);
...
fn test_render_list_joins_a_whole_multivalued_field() {
    ...
    let lines = report::render_list(&db, "USERS", &["ROLES".to_string()], None, &rows);
...
fn test_render_list_converts_each_value_of_a_multivalued_column() {
    ...
    let lines = report::render_list(&db, "USERS", &["PRICE".to_string()], None, &rows);
    ...
    let lines = report::render_list(&db, "USERS", &["PRICE".to_string()], None, &single);
...
fn test_render_list_expands_the_wildcard_column() {
    ...
    let lines = report::render_list(&db, "USERS", &["*".to_string()], None, &rows);
...
fn test_render_list_survives_a_stale_position() {
    ...
    let lines = report::render_list(&db, "USERS", &["ROLES".to_string()], Some("ROLES"), &rows);
