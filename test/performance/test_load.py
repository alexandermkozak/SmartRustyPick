...
def test_performance():
    ...
    try:
        ...
        for i in range(num_records):
            run_request(port, {"command": "WRITE", "table": "PERF", "key": f"REC{i}", "data": f"Val1^Val2^{i}"}, "client.crt", "client.key", "ca.crt")
        end_time = time.time()
        print(f"Time to write {num_records} records: {end_time - start_time:.2f}s")

        print("Testing simple query performance...")
        start_time = time.time()
        resp = run_request(port, {"command": "QUERY", "table": "PERF", "query_string": "WITH 3 = 500"}, "client.crt", "client.key", "ca.crt")
        end_time = time.time()
        ...
        start_time = time.time()
        resp = run_request(port, {"command": "QUERY", "table": "PERF", "query_string": "WITH 1 = Val1 AND 3 > 900"}, "client.crt", "client.key", "ca.crt")
        end_time = time.time()
